...
object HttpMethod { 
  ...
  def fromString(method: String): Option[HttpMethod] =
    httpMethods find { met => met.toString.toUpperCase == method.toUpperCase }
}
...
abstract class Response extends Serializable {
  ...
  def toServletResponse(req: HttpServletRequest, resp: HttpServletResponse) { 
    statusLine match { 
...
class RedirectResponse(destinationUrl: String, val statusCode: Int = 301) extends Response {
  ...
  override def toServletResponse(req: HttpServletRequest, resp: HttpServletResponse) { 
    outputHeaders(resp)
...
object BlobResponse { 
  def apply(contentType: String, blobKey: BlobKey, fileName: Option[String] = None) = 
    new BlobResponse(contentType, blobKey, fileName)
}

class BlobResponse(contentType: String, blobKey: BlobKey, fileName: Option[String]=None) 
